...
function onOpen() {
 ...
 function startUpMessage() {
  ...
  let buttonPressed = ui.alert('Please update status for all deliveries in DD sheet prior. Begin DD automation?',ui.ButtonSet.YES_NO);
...
function onEdit(event) {
  ...
  const columnNames = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues().shift();
...
if(sheet.getName() == SHEET.intakeForm){
    ...
    if(cellValue == STATUS.pendingDelivery){
      ...
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET.deliveries).appendRow(sheet.getRange(cellR, 1, 1, sheet.getLastColumn()).getValues()[0]);
      return;
    }
    ...
    if(cellValue == STATUS.closed) {
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET.intakeForm).getRange(cellR, getColIndex(COLUMN.dateCompleted)).setValue(new Date());
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET.closedCompleted).appendRow(sheet.getRange(cellR, 1, 1, 50).getValues()[0])
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET.intakeForm).deleteRows(cellR);
      return;
    }
...
//if DD sheet status is set to delivered, changes status to delivered in New Intake, deletes from NI and D tab and copies to closed/completed
  for (d=1; d < spreadsheet.getSheetByName(SHEET.deliveries).getLastRow(); d++){
      if(spreadsheet.getName() == SHEET.deliveries){;
                                                    if(spreadsheet.getRange(d, 3) == STATUS.delivered){
             ddRows.push(spreadsheet.getSheetByName(SHEET.deliveries).getRange(d,1).getValue());
             ddRows2.push(d);
             // the [0] bit is changing a Matrix to a list because getRange().getValues returns a section of the sheet not a row
             spreadsheet.getSheetByName(SHEET.closedCompleted).appendRow(spreadsheet.getSheetByName(SHEET.deliveries).getRange(d, 1, 1, spreadsheet.getSheetByName(SHEET.deliveries).getLastColumn()).getValues()[0]);
  ...
  for (n=1; n < spreadsheet.getsheetbyname(SHEET.intakeForm).getLastRow(); n++){
      for (let deliveredUID of ddRows){
        if (spreadsheet.getSheetByName(SHEET.intakeForm).getRange(n, 1).getValue() == deliveredUID){
...
function getHeaderRow() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET.deliveries);
...
function setDateForRows(rows) {
  ...
  for (r of rows) {
    ...
    r[dateIdx].setHours(timeVal.getHours(), timeVal.getMinutes(), timeVal.getSeconds());
  }
}
